...
object GeneralUtils {
  ...
  fun Context.generatePaletteFromAlbumArt(albumId: Long, listener: Palette.PaletteAsyncListener) {
    val target = object : CustomTarget<Drawable>() {
      override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable>?) {
        Palette.from(resource.toBitmap(200, 200)).generate(listener)
      }
    ...
    Glide.with(this).load(getAlbumArtUri(albumId)).into(target)
  }

  fun Fragment.generatePaletteFromAlbumArt(albumId: Long, listener: Palette.PaletteAsyncListener) {
    requireActivity().generatePaletteFromAlbumArt(albumId, listener)
  }
